...
// SPDX-FileCopyrightText: (C) 1997  Curtis L. Olson - http://www.flightgear.org/~curt
...
// Constructor
FGControls::FGControls() : flaps(0.0),
                           slats(0.0),
                           antiskid(true),
                           tailhook(false),
                           launchbar(false),
...
void FGControls::reset_all()
{
    ...
    brake_left = brake_right = copilot_brake_left = copilot_brake_right = brake_parking = 0.0;
...
void FGControls::bind()
{
    ...
    _rudderTrimNode = _tiedProperties.Tie("rudder-trim", this, &FGControls::get_rudder_trim, &FGControls::set_rudder_trim);
    _flapsNode = _tiedProperties.Tie("flaps", this, &FGControls::get_flaps, &FGControls::set_flaps);
    ...
    for (index = 0; index < MAX_ENGINES; index++) {
        ...
        _enginePropAdvanceNodes.push_back(_tiedProperties.Tie("propeller-pitch", this, index, &FGControls::get_prop_advance, &FGControls::set_prop_advance));
        ...
        _engineConditionNodes.push_back(_tiedProperties.Tie("condition", this, index, &FGControls::get_condition, &FGControls::_inner_set_condition));
    }
    ...
    // we can do this because we made all the accessors private
...
void FGControls::unbind()
{
    ...
    _engineConditionNodes.clear();
    _enginePropAdvanceNodes.clear();
}
...
void FGControls::move_aileron(double amt)
{
    _inner_set_aileron(aileron + amt);
    _aileronNode->fireValueChanged();
}
...
void FGControls::move_aileron_trim(double amt)
{
    set_aileron_trim(aileron_trim + amt);
}
...
void FGControls::move_elevator(double amt)
{
    set_elevator(elevator + amt);
}
...
void FGControls::_inner_set_elevator_trim(double pos)
{
    if (std::isnan(pos)) {
        throw sg_range_exception("FGControls::_inner_set_elevator_trim: passed NaN");
    }
...
void FGControls::move_elevator_trim(double amt)
{
    set_elevator_trim(elevator_trim + amt);
}
...
void FGControls::move_rudder(double amt)
{
    set_rudder(rudder + amt);
}
...
void FGControls::set_rudder_trim(double pos)
{
    _inner_set_rudder_trim(pos);
    if (_rudderTrimNode) {
        _rudderTrimNode->fireValueChanged();
    }
}

void FGControls::move_rudder_trim(double amt)
{
    set_rudder_trim(rudder_trim + amt);
}
...
void FGControls::set_flaps(double pos)
{
    _inner_set_flaps(pos);
    if (_flapsNode) {
        _flapsNode->fireValueChanged();
    }
}

void FGControls::move_flaps(double amt)
{
    set_flaps(flaps + amt);
}

void FGControls::set_slats(double pos)
{
    slats = pos;
    SG_CLAMP_RANGE<double>(slats, 0.0, 1.0);
}

void FGControls::move_slats(double amt)
{
    slats += amt;
    SG_CLAMP_RANGE<double>(slats, 0.0, 1.0);
}

void FGControls::set_BLC(bool val)
{
    BLC = val;
}

void FGControls::set_spoilers(double pos)
{
    spoilers = pos;
    SG_CLAMP_RANGE<double>(spoilers, 0.0, 1.0);
}

void FGControls::move_spoilers(double amt)
{
    spoilers += amt;
    SG_CLAMP_RANGE<double>(spoilers, 0.0, 1.0);
}

void FGControls::set_speedbrake(double pos)
{
    speedbrake = pos;
    SG_CLAMP_RANGE<double>(speedbrake, 0.0, 1.0);
}

void FGControls::move_speedbrake(double amt)
{
    speedbrake += amt;
    SG_CLAMP_RANGE<double>(speedbrake, 0.0, 1.0);
}

void FGControls::set_wing_sweep(double pos)
{
    wing_sweep = pos;
    SG_CLAMP_RANGE<double>(wing_sweep, 0.0, 1.0);
}

void FGControls::move_wing_sweep(double amt)
{
    wing_sweep += amt;
    SG_CLAMP_RANGE<double>(wing_sweep, 0.0, 1.0);
}

void FGControls::set_wing_fold(bool val)
{
    wing_fold = val;
}

void FGControls::set_drag_chute(bool val)
{
    drag_chute = val;
}

void FGControls::set_throttle_idle(bool val)
{
    throttle_idle = val;
}

void FGControls::set_throttle(int engine, double pos)
{
...
void FGControls::_inner_set_starter(int engine, bool flag)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            starter[i] = flag;
...
void FGControls::set_fuel_pump(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            fuel_pump[i] = val;
...
void FGControls::set_fire_switch(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            fire_switch[i] = val;
...
void FGControls::set_fire_bottle_discharge(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
...
void FGControls::set_cutoff(int engine, bool val)
{
    _inner_set_cutoff(engine, val);
    fireEngineValueChanged(engine, _engineCutoffNodes);
}

void FGControls::_inner_set_cutoff(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            cutoff[i] = val;
...
void FGControls::set_feed_tank(int engine, int tank)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            feed_tank[i] = tank;
...
void FGControls::set_mixture(int engine, double pos)
{
...
void FGControls::set_prop_advance(int engine, double pos)
{
    ...
    fireEngineValueChanged(engine, _enginePropAdvanceNodes);
}
...
void FGControls::set_magnetos(int engine, int pos)
{
...
void FGControls::_inner_set_magnetos(int engine, int pos)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            magnetos[i] = pos;
...
void FGControls::move_magnetos(int engine, int amt)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            magnetos[i] += amt;
...
void FGControls::set_nitrous_injection(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
...
void FGControls::set_cowl_flaps_norm(int engine, double pos)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
...
void FGControls::move_cowl_flaps_norm(int engine, double amt)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
...
void FGControls::set_feather(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            feather[i] = val;
...
void FGControls::set_ignition(int engine, int pos)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            ignition[i] = pos;
...
void FGControls::set_augmentation(int engine, bool val)
{
    _inner_set_augmentation(engine, val);
    fireEngineValueChanged(engine, _engineAugmentationNodes);
}

void FGControls::_inner_set_augmentation(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            augmentation[i] = val;
...
void FGControls::set_reverser(int engine, bool val)
{
    _inner_set_reverser(engine, val);
    fireEngineValueChanged(engine, _engineReverserNodes);
}

void FGControls::_inner_set_reverser(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            reverser[i] = val;
...
void FGControls::set_water_injection(int engine, bool val)
{
    _inner_set_water_injection(engine, val);
    fireEngineValueChanged(engine, _engineWaterInjectionNodes);
}

void FGControls::_inner_set_water_injection(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
...
void FGControls::set_condition(int engine, double val)
{
    _inner_set_condition(engine, val);
    fireEngineValueChanged(engine, _engineConditionNodes);
}
...
void FGControls::set_dump_valve(bool val)
{
    dump_valve = val;
}


void FGControls::set_fuel_selector(int tank, bool pos)
{
    if (tank == ALL_TANKS) {
...
void FGControls::set_to_engine(int tank, int engine)
{
    if (tank == ALL_TANKS) {
        for (int i = 0; i < MAX_TANKS; i++) {
            to_engine[i] = engine;
...
void FGControls::set_to_tank(int tank, int dest_tank)
{
    if (tank == ALL_TANKS) {
        for (int i = 0; i < MAX_TANKS; i++) {
            to_tank[i] = dest_tank;
...
void FGControls::set_brake_left(double pos)
{
    brake_left = pos;
    SG_CLAMP_RANGE<double>(brake_left, 0.0, 1.0);
}

void FGControls::move_brake_left(double amt)
{
    brake_left += amt;
    SG_CLAMP_RANGE<double>(brake_left, 0.0, 1.0);
}

void FGControls::set_brake_right(double pos)
{
    brake_right = pos;
    SG_CLAMP_RANGE<double>(brake_right, 0.0, 1.0);
}

void FGControls::move_brake_right(double amt)
{
    brake_right += amt;
    SG_CLAMP_RANGE<double>(brake_right, 0.0, 1.0);
}

void FGControls::set_copilot_brake_left(double pos)
{
    copilot_brake_left = pos;
    SG_CLAMP_RANGE<double>(brake_left, 0.0, 1.0);
}

void FGControls::set_copilot_brake_right(double pos)
{
    copilot_brake_right = pos;
    SG_CLAMP_RANGE<double>(brake_right, 0.0, 1.0);
}

void FGControls::set_brake_parking(double pos)
{
    brake_parking = pos;
    SG_CLAMP_RANGE<double>(brake_parking, 0.0, 1.0);
}

void FGControls::set_steering(double angle)
{
    steering = angle;
    SG_CLAMP_RANGE<double>(steering, -80.0, 80.0);
}

void FGControls::set_nose_wheel_steering(bool nws)
{
    nose_wheel_steering = nws;
}

void FGControls::move_steering(double angle)
{
    steering += angle;
    SG_CLAMP_RANGE<double>(steering, -80.0, 80.0);
}

void FGControls::set_gear_down(bool gear)
{
    gear_down = gear;
}

void FGControls::set_antiskid(bool state)
{
    antiskid = state;
}

void FGControls::set_tailhook(bool state)
{
    tailhook = state;
}

void FGControls::set_launchbar(bool state)
{
    launchbar = state;
}

void FGControls::set_catapult_launch_cmd(bool state)
{
    catapult_launch_cmd = state;
}

void FGControls::set_tailwheel_lock(bool state)
{
    tailwheel_lock = state;
}


void FGControls::set_alternate_extension(int wheel, bool val)
{
    if (wheel == ALL_WHEELS) {
...
void FGControls::set_wing_heat(bool state)
{
    wing_heat = state;
}

void FGControls::set_pitot_heat(bool state)
{
    pitot_heat = state;
}

void FGControls::set_wiper(int state)
{
    wiper = state;
}

void FGControls::set_window_heat(bool state)
{
    window_heat = state;
}

void FGControls::set_carb_heat(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            carb_heat[i] = val;
...
void FGControls::set_inlet_heat(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            inlet_heat[i] = val;
...
void FGControls::set_engine_pump(int system, bool val)
{
...
void FGControls::set_electric_pump(int system, bool val)
{
    if (system == ALL_HYD_SYSTEMS) {
        for (int i = 0; i < MAX_HYD_SYSTEMS; i++) {
            electric_pump[i] = val;
...
void FGControls::set_battery_switch(bool state)
{
    battery_switch = state;
}

void FGControls::set_external_power(bool state)
{
    external_power = state;
}

void FGControls::set_APU_generator(bool state)
{
    APU_generator = state;
}

void FGControls::set_generator_breaker(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
...
void FGControls::set_bus_tie(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            bus_tie[i] = val;
...
void FGControls::set_APU_bleed(bool state)
{
    APU_bleed = state;
}

void FGControls::set_engine_bleed(int engine, bool val)
{
    if (engine == ALL_ENGINES) {
        for (int i = 0; i < MAX_ENGINES; i++) {
            engine_bleed[i] = val;
...
void FGControls::set_mode(int new_mode)
{
    mode = new_mode;
}

void FGControls::set_outflow_valve(double pos)
{
    outflow_valve = pos;
    SG_CLAMP_RANGE<double>(outflow_valve, 0.0, 1.0);
}

void FGControls::move_outflow_valve(double amt)
{
    outflow_valve += amt;
    SG_CLAMP_RANGE<double>(outflow_valve, 0.0, 1.0);
}

void FGControls::set_dump(bool state)
{
    dump = state;
}

void FGControls::set_pack_on(int pack, bool val)
{
    if (pack == ALL_PACKS) {
...
void FGControls::set_landing_lights(bool state)
{
    landing_lights = state;
}

void FGControls::set_turn_off_lights(bool state)
{
    turn_off_lights = state;
}

void FGControls::set_taxi_light(bool state)
{
    taxi_light = state;
}

void FGControls::set_logo_lights(bool state)
{
    logo_lights = state;
}

void FGControls::set_nav_lights(bool state)
{
    nav_lights = state;
}

void FGControls::set_beacon(bool state)
{
    beacon = state;
}

void FGControls::set_strobe(bool state)
{
    strobe = state;
}

void FGControls::set_panel_norm(double intensity)
{
    panel_norm = intensity;
    SG_CLAMP_RANGE<double>(panel_norm, 0.0, 1.0);
}

void FGControls::move_panel_norm(double amt)
{
    panel_norm += amt;
    SG_CLAMP_RANGE<double>(panel_norm, 0.0, 1.0);
}

void FGControls::set_instruments_norm(double intensity)
{
    instruments_norm = intensity;
    SG_CLAMP_RANGE<double>(instruments_norm, 0.0, 1.0);
}

void FGControls::move_instruments_norm(double amt)
{
    instruments_norm += amt;
    SG_CLAMP_RANGE<double>(instruments_norm, 0.0, 1.0);
}

void FGControls::set_dome_norm(double intensity)
{
    dome_norm = intensity;
    SG_CLAMP_RANGE<double>(dome_norm, 0.0, 1.0);
}

void FGControls::move_dome_norm(double amt)
{
    dome_norm += amt;
    SG_CLAMP_RANGE<double>(dome_norm, 0.0, 1.0);
}

void FGControls::set_master_arm(bool val)
{
    master_arm = val;
}

void FGControls::set_station_select(int station)
{
    station_select = station;
    SG_CLAMP_RANGE<int>(station_select, 0, MAX_STATIONS);
}

void FGControls::set_release_ALL(bool val)
{
    release_ALL = val;
}

void FGControls::set_stick_size(int station, int size)
{
...
void FGControls::set_release_stick(int station, bool val)
{
    if (station == ALL_STATIONS) {
        for (int i = 0; i < MAX_STATIONS; i++) {
            release_stick[i] = val;
...
void FGControls::set_release_all(int station, bool val)
{
    if (station == ALL_STATIONS) {
        for (int i = 0; i < MAX_STATIONS; i++) {
            release_all[i] = val;
...
void FGControls::set_jettison_all(int station, bool val)
{
    if (station == ALL_STATIONS) {
        for (int i = 0; i < MAX_STATIONS; i++) {
            jettison_all[i] = val;
...
void FGControls::set_vertical_adjust(double pos)
{
    vertical_adjust = pos;
    SG_CLAMP_RANGE<double>(vertical_adjust, -1.0, 1.0);
}

void FGControls::move_vertical_adjust(double amt)
{
    vertical_adjust += amt;
    SG_CLAMP_RANGE<double>(vertical_adjust, -1.0, 1.0);
}

void FGControls::set_fore_aft_adjust(double pos)
{
    fore_aft_adjust = pos;
    SG_CLAMP_RANGE<double>(fore_aft_adjust, -1.0, 1.0);
}

void FGControls::move_fore_aft_adjust(double amt)
{
    fore_aft_adjust += amt;
    SG_CLAMP_RANGE<double>(fore_aft_adjust, -1.0, 1.0);
}

void FGControls::set_ejection_seat(int which_seat, bool val)
{
...
void FGControls::set_eseat_status(int which_seat, int val)
{
    if (which_seat == ALL_EJECTION_SEATS) {
        for (int i = 0; i < MAX_EJECTION_SEATS; i++) {
            eseat_status[i] = val;
...
void FGControls::set_cmd_selector_valve(int val)
{
    cmd_selector_valve = val;
}


void FGControls::set_off_start_run(int pos)
{
    off_start_run = pos;
    SG_CLAMP_RANGE<int>(off_start_run, 0, 3);
}

void FGControls::set_APU_fire_switch(bool val)
{
    APU_fire_switch = val;
}

void FGControls::set_autothrottle_arm(bool val)
{
    autothrottle_arm = val;
}

void FGControls::set_autothrottle_engage(bool val)
{
    autothrottle_engage = val;
}

void FGControls::set_heading_select(double heading)
{
    heading_select = heading;
    SG_CLAMP_RANGE<double>(heading_select, 0.0, 360.0);
}

void FGControls::move_heading_select(double amt)
{
    heading_select += amt;
    SG_CLAMP_RANGE<double>(heading_select, 0.0, 360.0);
}

void FGControls::set_altitude_select(double altitude)
{
    altitude_select = altitude;
    SG_CLAMP_RANGE<double>(altitude_select, -1000.0, 100000.0);
}

void FGControls::move_altitude_select(double amt)
{
    altitude_select += amt;
    SG_CLAMP_RANGE<double>(altitude_select, -1000.0, 100000.0);
}

void FGControls::set_bank_angle_select(double angle)
{
    bank_angle_select = angle;
    SG_CLAMP_RANGE<double>(bank_angle_select, 10.0, 30.0);
}

void FGControls::move_bank_angle_select(double amt)
{
    bank_angle_select += amt;
    SG_CLAMP_RANGE<double>(bank_angle_select, 10.0, 30.0);
}

void FGControls::set_vertical_speed_select(double speed)
{
    vertical_speed_select = speed;
    SG_CLAMP_RANGE<double>(vertical_speed_select, -3000.0, 4000.0);
}

void FGControls::move_vertical_speed_select(double amt)
{
    vertical_speed_select += amt;
    SG_CLAMP_RANGE<double>(vertical_speed_select, -3000.0, 4000.0);
}

void FGControls::set_speed_select(double speed)
{
    speed_select = speed;
    SG_CLAMP_RANGE<double>(speed_select, 60.0, 400.0);
}

void FGControls::move_speed_select(double amt)
{
    speed_select += amt;
    SG_CLAMP_RANGE<double>(speed_select, 60.0, 400.0);
}

void FGControls::set_mach_select(double mach)
{
    mach_select = mach;
    SG_CLAMP_RANGE<double>(mach_select, 0.4, 4.0);
}

void FGControls::move_mach_select(double amt)
{
    mach_select += amt;
    SG_CLAMP_RANGE<double>(mach_select, 0.4, 4.0);
}

void FGControls::set_vertical_mode(int mode)
{
    vertical_mode = mode;
    SG_CLAMP_RANGE<int>(vertical_mode, 0, 4);
}

void FGControls::set_lateral_mode(int mode)
{
    lateral_mode = mode;
    SG_CLAMP_RANGE<int>(lateral_mode, 0, 4);
}

void FGControls::set_autopilot_engage(int ap, bool val)
{
...
void FGControls::fireEngineValueChanged(int index, simgear::PropertyList& props)
{
    ...
    } else if ((index >= 0) && (index < MAX_ENGINES) && (index < props.size())) {
