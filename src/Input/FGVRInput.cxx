      ...
      _node(node)
{
    ...
    // localize name
    std::string locId = "actionset-" + name;
    std::string nameLoc = globals->get_locale()->getLocalizedString(locId.c_str(), "vr", desc.c_str());

    setName(name, nameLoc);
    if (priority >= 0)
...
FGVRInput::ActionCommon::ActionCommon(osgXR::ActionSet* actionSet,
                                      osgXR::Action* action,
                                      SGPropertyNode* node)
    : _node(node)
{
    ...
    // localize name
    std::string locId = "actionset-" + actionSet->getName() + "-action-" + name;
    std::string nameLoc = globals->get_locale()->getLocalizedString(locId.c_str(), "vr", desc.c_str());

    action->setName(name, nameLoc);
}
...
FGVRInput::SubactionInfoVector2f::SubactionInfoVector2f(osgXR::Action* action,
    ...
    : SubactionInfo(action, subaction, node),
      ...
      _statusProp{SGPropObjDouble(statusNode->getChild("x", 0, true)),
...
FGVRInput::SubactionInfoPose::SubactionInfoPose(osgXR::Action* action,
    ...
    : SubactionInfo(action, subaction, node),
      ...
      _statusPositionProp{SGPropObjDouble(_statusPositionNode->getChild("x", 0, true)),
...
static osgXR::Condition *generateCondition(osgXR::Manager* manager,
                                           SGPropertyNode *node)
{
    SGPropertyNode *apiVersionNode = node->getChild("api-version");
    SGPropertyNode *extensionNode = node->getChild("extension");
    if (!apiVersionNode && !extensionNode)
        return nullptr;

    auto *condition = new osgXR::ConditionApi();
    if (apiVersionNode) {
        int majorVersion = apiVersionNode->getIntValue("major", 0);
        int minorVersion = apiVersionNode->getIntValue("minor", 0);
        osgXR::Version apiVersion(majorVersion, minorVersion);
        condition->setApiVersion(apiVersion);
    }
    if (extensionNode) {
        auto extension = manager->getExtension(extensionNode->getStringValue());
        condition->setExtension(extension);
    }
...
{
    // Read the overall condition
    SGPropertyNode* conditionNode = node->getNode("condition");
    if (conditionNode) {
        auto *condition = generateCondition(manager, conditionNode);
        if (condition)
    ...
    for (int i = 0; i < node->nChildren(); ++i) {
        ...
        if (it != input->_actionSets.end()) {
            ...
            for (int j = 0; j < actionSetNode->nChildren(); ++j) {
                ...
                if (action) {
                    // Read condition for this action
                    conditionNode = actionNode->getChild("condition");
                    osg::ref_ptr<osgXR::Condition> condition;
                    if (conditionNode)
                        condition = generateCondition(manager, conditionNode);
                    ...
                    for (auto bindingNode : actionNode->getChildren("binding")) {
                        ...
                        suggestBinding(action, binding, condition);
                    }
                }
            }
        }
...
FGVRInput::ModeProcessInputSourceAction::ModeProcessInputSourceAction(Subaction* subaction,
                                                                      osgXR::Action* action)
    : _subaction(subaction),
      _action(action)
{
      ...
      _lastVec2f(0.0f, 0.0f)
{
    for (SGPropertyNode* actionNode : node->getChildren("action")) {
        ...
        if (action) {
            _sources.push_back(std::make_unique<ModeProcessInputSourceAction>(subaction,
                                                                              action));
        } else {
            SG_LOG(SG_INPUT, SG_WARN,
                      ...
                      SGPropertyNode* dataNode)
{
    ...
    for (unsigned int i = 0; i < subactions.size(); ++i) {
        ...
        if (!subactionPath.empty()) {
            ...
            info->readProcesses(this, nodeCopy, statusNode);

            // And subaction specific process objects
            info->readProcesses(this, subactionCopy, statusNode);
        }
    }
}
