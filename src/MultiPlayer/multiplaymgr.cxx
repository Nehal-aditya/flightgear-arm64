   ...
   const simgear::IPAddress& SenderAddress, long stamp)
{
   ...
   // End of the data actually received used for loop
   // and to ensure that string sizes do not overflow the buffer
   const xdr_data_t* const packetEnd = Msg.propsRecvdEnd();
   ...
   while (xdr < packetEnd) {
       ...
       if (pMultiPlayDebugLevel->getIntValue() & 8)
           SG_LOG(SG_NETWORK, SG_INFO,
                  "[RECV] add " << std::hex << xdr
                                << std::dec << ": buf[" << ((char*)xdr) - ((char*)data)
                                << "] id=" << id
       ...
       if (plist) {
           ...
           if (plist->decode_received) {
               ...
               */
               ...
               case simgear::props::UNSPECIFIED: {
                   ...
                   if (short_int_encoded) {
                       // A short-int-encoded string carries its length in int_value,
                       // taken straight off the wire and could be manipulated to have
                       // a value that could cause bad things to happen (see #3472)
                       // So we validate the length is positive and doesn't exceed
                       // the maximum permitted size or the end of packet.
                       // If the length fails validation then we need to bail out now
                       // because the packet is badly formed.
                       uint32_t length = (uint32_t)int_value;
                       if (int_value < 0 || length > MAX_TEXT_SIZE ||
                           length > (size_t)(packetEnd - xdr) * sizeof(xdr_data_t)) {
                           delete pData;
                           goto noprops;
                       }
           ...
           SG_LOG(SG_NETWORK, SG_DEBUG, "FGMultiplayMgr::ProcessPosMsg - "
                                        "message from "
                                            << MsgHdr->Callsign << " has unknown property id " << id);
