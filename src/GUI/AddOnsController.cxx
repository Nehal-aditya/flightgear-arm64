...
void AddOnsController::setLocalAircraftPaths()
{
    ...
    for (const auto& arg : commandLineAircraftPaths) {
        // inner loop because a single arg can define multiple paths
...
QString AddOnsController::addSceneryPath() const
{
    ...
    for (const auto& dir: {"Objects", "Terrain", "Buildings", "Roads", "Pylons", "NavData", "Airports", "Orthophotos", "vpb"}) {
        if ((p / dir).exists()) {
            isValid = true;
            break;
        }
    ...
    if (!isValid) {
        ...
        mb.setInformativeText(tr("Added scenery should contain at least one of the following "
                                 "folders: Objects, Terrain, Buildings, Roads, Pylons, NavData, Airports, Orthophotos, vpb."));
        mb.exec();
...
void AddOnsController::collectArgs()
{
    ...
    for (const auto& catRef : globals->packageRoot()->catalogs()) {
        const auto catAircraftPath = catRef->installRoot() / "Aircraft";
        m_config->setArg("fg-aircraft", QString::fromStdString(catAircraftPath.utf8Str()));
    }
