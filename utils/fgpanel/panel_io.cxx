 ...
 * positions.  Every layer starts centered exactly on the x,y coordinate,
  ...
  if (trans_table != 0) {
    ...
    for (int i = 0; i < trans_table->nChildren(); i++) {
      ...
      if (node->getNameString() == "entry") {
          ...
          SG_LOG(SG_COCKPIT, SG_INFO, "Adding interpolation entry " << ind << "==>" << dep);
          t->table->addEntry(ind, dep);
      } else {
          SG_LOG(SG_COCKPIT, SG_INFO, "Skipping " << node->getNameString() << " in interpolation");
      }
    }
  } else {
    t->table = 0;
    ...
    if (chunk_group != 0) {
      ...
      for (int i = 0; i < nChunks; i++) {
        ...
        if (node->getNameString() == "chunk") {
            ...
            SG_LOG(SG_COCKPIT, SG_INFO, "Skipping " << node->getNameString() << " in chunks");
        }
      }
      layer = tlayer;
    }
  ...
  if (trans_group != 0) {
    ...
    for (int i = 0; i < nTransformations; i++) {
      ...
      if (node->getNameString() == "transformation") {
          ...
          SG_LOG(SG_COCKPIT, SG_INFO, "Skipping " << node->getNameString() << " in transformations");
      }
    }
  }
 ...
 * the mouse over the instrument.  All coordinates are relative
 * to the instrument's position, so instruments are fully relocatable;
 * likewise, coordinates for actions and transformations will be
  ...
  if (layer_group != 0) {
    ...
    for (int i = 0; i < nLayers; i++) {
      ...
      if (node->getNameString() == "layer") {
          ...
          SG_LOG(SG_COCKPIT, SG_INFO, "Skipping " << node->getNameString() << " in layers");
      }
    }
  }
  ...
  if (instrument_group != 0) {
    ...
    for (int i = 0; i < nInstruments; i++) {
      ...
      if (node->getNameString() == "instrument") {
          ...
          SG_LOG(SG_COCKPIT, SG_INFO, "Skipping " << node->getNameString() << " in instruments section");
      }
    }
  }
